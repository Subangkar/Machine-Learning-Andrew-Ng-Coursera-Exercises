...
% y a column vector each row denotes which class it's from
y = repmat(1:num_labels, m, 1) == repmat(y, 1, num_labels);
% y each row a datapoint @colm(k)==1 means it's of class=k
cost = @(a3,y) (-y.*log(a3)-(1-y).*log(1-a3));
...
Theta2(:,1) = 0;
error = cost(a3,y');
J = (1/(m))*sum(error(:)) + (lambda/(2*m))*(sum(Theta1(:).^2)+sum(Theta2(:).^2));
